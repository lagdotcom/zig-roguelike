// source: https://github.com/Jack-Ji/jok/blob/main/src/utils/pathfind.zig
...
//     pub fn iterate_neighbours(self: @This(), id: usize) Iterator
//
...
inline fn verify_graph_structure(graph: anytype) void {
    const gtype = @TypeOf(graph);
    if (!@hasDecl(gtype, "iterate_neighbours") or
    ...
    switch (@typeInfo(@typeInfo(@TypeOf(gtype.iterate_neighbours)).Fn.return_type.?)) {
        .Struct => if (!std.meta.hasMethod(@typeInfo(@TypeOf(gtype.iterate_neighbours)).Fn.return_type.?, "next")) {
            @compileError("`iterate_neighbours` must return a valid iterator");
        },
        else => @compileError("`iterate_neighbours` must return Iterator"),
    }
...
pub fn calculate_path(allocator: std.mem.Allocator, graph: anytype, from: usize, to: usize) !?std.ArrayList(usize) {
    ...
    while (frontier.count() != 0) {
        ...
        var it = graph.iterate_neighbours(current);
        while (it.next()) |next| {
