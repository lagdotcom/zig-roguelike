...
pub const Terminal = struct {
    ...
    pub fn setCursorVisible(self: *Terminal, show: bool) !void {
    ...
    pub fn printAt(self: *Terminal, x: i16, y: i16, comptime format: []const u8, args: anytype) !void {
        try self.at(x, y);
        try self.buffer.writer().print(format, args);
    }

    pub fn setChar(self: *Terminal, x: i16, y: i16, fg: RGB8, bg: RGB8, ch: u8) !void {
    ...
    pub fn resetColour(self: *Terminal) !void {
        try self.buffer.writer().print(ansi.sgr, .{@intFromEnum(ansi.sgr_type.reset)});
    }

    pub fn setForegroundColour(self: *Terminal, rgb: RGB8) !void {
        try self.buffer.writer().print(ansi.sgr_foreground_rgb, .{ rgb.r, rgb.g, rgb.b });
    }

    pub fn setBackgroundColour(self: *Terminal, rgb: RGB8) !void {
    ...
    pub fn drawRect(self: *Terminal, x: i16, y: i16, width: usize, height: usize, ch: u8) !void {
        for (0..height) |yo| {
