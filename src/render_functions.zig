...
pub fn render_bar(terminal: *Terminal, current: i16, maximum: i16, total_width: usize) !void {
    ...
    try terminal.set_background_colour(col.bar_empty);
    try terminal.draw_rectangle(0, 45, total_width, 1, ' ');
    if (bar_width > 0) {
        try terminal.set_background_colour(col.bar_filled);
        try terminal.draw_rectangle(0, 45, bar_width, 1, ' ');
    }

    try terminal.reset_colour();
    try terminal.set_foreground_colour(col.bar_text);
    try terminal.print_at(1, 45, "HP: {}/{}", .{ current, maximum });
}

pub fn render_box(terminal: *Terminal, x: i16, y: i16, w: usize, h: usize, fg: col.RGB8, bg: col.RGB8) !void {
